...
public interface SyncService {
    ...
    default boolean needSync(Instance instanceInfo) {
        Boolean status = Optional.ofNullable(instanceInfo).map(i -> i.isHealthy()).get();
    ...
    default boolean needSync(InstanceInfo instanceInfo) {
        InstanceInfo.InstanceStatus status = Optional.ofNullable(instanceInfo).map(i -> i.getStatus()).get();
        if (status != null && status.equals(InstanceInfo.InstanceStatus.UP)) {
            return true;
        }
        return false;
    }
