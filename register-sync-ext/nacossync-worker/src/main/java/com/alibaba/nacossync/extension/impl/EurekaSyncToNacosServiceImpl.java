...
public class EurekaSyncToNacosServiceImpl implements SyncService {
    ...
    public boolean deleteBeat(TaskDO taskDO) {

        try {
            NacosNamingService destNamingService =
                    (NacosNamingService) nacosServerHolder.get(taskDO.getDestClusterId(), taskDO.getNameSpace());
            List<Instance> allInstances = destNamingService.getAllInstances(taskDO.getServiceName());
            deleteAllInstanceBeat(taskDO, destNamingService, allInstances);
    ...
    private void addValidInstance(TaskDO taskDO, NamingService destNamingService, List<InstanceInfo> eurekaInstances)
        ...
        for (InstanceInfo instance : eurekaInstances) {
            if (needSync(instance.getMetadata()) && needSync(instance)) {
    ...
    private boolean isExistInEurekaInstance(List<InstanceInfo> eurekaInstances, Instance nacosInstance) {
        ...
        return eurekaInstances.stream().anyMatch(instance -> instance.getIPAddr().equals(nacosInstance.getIp())
                && instance.getPort() == nacosInstance.getPort() && instance.getStatus().equals(InstanceInfo.InstanceStatus.UP));
    }
    ...
    private void deleteAllInstanceBeat(TaskDO taskDO, NacosNamingService destNamingService, List<Instance> allInstances)
            throws NacosException {
        for (Instance instance : allInstances) {
            if (needDelete(instance.getMetadata(), taskDO)) {
                destNamingService.getBeatReactor().removeBeatInfo(instance.getServiceName(), instance.getIp(),
                        instance.getPort());
            }
        }
    }
