...
public class TaskApi {
    ...
    @Autowired
    private TaskInitProcessor taskInitProcessor;

    @Autowired
    private TaskInvalidateProcessor taskInvalidateProcessor;
    ...
    @RequestMapping(path = "/v1/task/init", method = RequestMethod.GET)
    public BaseResult initTask(TaskInitRequest taskInitRequest) {

        return SkyWalkerTemplate.run(taskInitProcessor, taskInitRequest, new BaseResult());
    }

    @RequestMapping(path = "/v1/task/invalidate", method = RequestMethod.GET)
    public BaseResult invalidateTask(TaskInvalidateRequest taskInvalidateRequest) {

        return SkyWalkerTemplate.run(taskInvalidateProcessor, taskInvalidateRequest, new BaseResult());
    }
}
